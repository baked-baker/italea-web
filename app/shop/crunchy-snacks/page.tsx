...
import { ShoppingCart, Heart, Star, Zap, Shield, Flame } from "lucide-react"
...
export default function CrunchySnacksPage() {
  const [hoveredProduct, setHoveredProduct] = useState<number | null>(null)
  const { addItem } = useCart()
  const products = getProductsByCategory("crunchy-snacks")

  const features = [
    {
      icon: <Zap className="w-6 h-6 text-yellow-600" />,
      title: "Energy Boost",
      description: "Perfect for pre-workout, hiking, or afternoon energy dips",
    },
    {
      icon: <Shield className="w-6 h-6 text-green-600" />,
      title: "Clean Ingredients",
      description: "No preservatives, artificial flavors, or unnecessary additives",
    },
    {
      icon: <Flame className="w-6 h-6 text-red-600" />,
      title: "Bold Flavors",
      description: "Naturally spiced and seasoned for maximum taste satisfaction",
    },
  ]
    ...
    <div className="min-h-screen bg-gradient-to-b from-red-50 to-orange-50">
      ...
      <div className="pt-20 pb-12">
        <div className="max-w-7xl mx-auto px-4">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-serif text-amber-900 mb-6">Crunchy Snacks</h1>
            <p className="text-xl text-amber-800 max-w-3xl mx-auto mb-8">
              Satisfy your cravings with our collection of wholesome, crunchy snacks. From spicy nuts to roasted seeds,
              each bite delivers bold flavors and clean nutrition.
            </p>
            <div className="flex flex-wrap justify-center gap-4 mb-12">
              <Badge className="bg-red-100 text-red-800 px-4 py-2">Naturally Spicy</Badge>
              <Badge className="bg-green-100 text-green-800 px-4 py-2">High Protein</Badge>
              <Badge className="bg-orange-100 text-orange-800 px-4 py-2">No Preservatives</Badge>
              <Badge className="bg-purple-100 text-purple-800 px-4 py-2">Energy Boost</Badge>
            </div>
          </motion.div>
                      ...
                      {product.nutritionalHighlights && (
                        <div className="flex flex-wrap gap-1 mb-4">
                          {product.nutritionalHighlights.slice(0, 3).map((highlight) => (
                            <Badge key={highlight} variant="outline" className="text-xs border-red-200 text-red-700">
                              {highlight}
                            </Badge>
                          ))}
                        </div>
          ...
          >
            <h2 className="text-3xl font-serif text-amber-900 mb-4">Fuel Your Adventures</h2>
            <p className="text-amber-700 mb-6 max-w-2xl mx-auto">
              Whether you're hitting the trails, powering through a workout, or need an afternoon pick-me-up, our
              crunchy snacks deliver the energy and satisfaction you crave.
            </p>
            <Link href="/shop">
